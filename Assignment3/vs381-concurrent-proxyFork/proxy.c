 ...
 *
 * This is the forked version of the provided proxy.c file
 *
 */ 

/*  This version of the proxy is very simple in implementation. The only difference
between this version and the basic sequential web proxy is the use of the fork function
to call the processRequest function and the flocks. The fork function is used to create
a new child process, so in my implementation, whenever a new request is accepted, a 
child process is created and runs the processRequest function.
    Similar to as described in the pthread explanation, everything after the accept
function is put into a function called processRequest and the arguments that need to
be passed in are the connection file descriptor (acquired from accept) and the
client address. When in the processRequest, an flock function is used to provide
synchronization when accessing the log file. Overall, this runs considerably faster than 
...
/*This is the function that is called whenever a child process is created. */
void processRequest(int connfd, struct sockaddr_in clientaddr){
...
int main(int argc, char **argv)
{
    ...
    while (1) { 
        ...
        if(fork() == 0){
            processRequest(connfd, clientaddr);
        }
